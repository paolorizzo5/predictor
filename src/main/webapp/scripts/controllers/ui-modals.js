...
app
  ...
  .controller('ModalSelectCompetitionCtrl', ['$scope','$uibModal','$log', 'AreaService', function ($scope, $uibModal, $log, AreaService) {
	  $scope.items = ['item1', 'item2', 'item3'];
	  if($scope.areas == null){
		AreaService.list($scope.configuration.serviceUrl).then(function(d) {
			if (d.data != null){
				$scope.areas = angular.fromJson(d.data);
			}else{
			}
		}); 
	  }
